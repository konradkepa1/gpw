...
public class przenikanie {

    public static String[] przenikanie() {
        ...
        for (int j = 0; j < rows; j++) {
            if (onlyDouble[1][j][3] < onlyDouble[1][j][0] && onlyDouble[0][j][0] < onlyDouble[1][j][3]
                    && onlyDouble[0][j][3] > onlyDouble[1][j][0] - (onlyDouble[1][j][0]-onlyDouble[1][j][3])/2) {
                roznica = onlyDouble[0][j][3] * 100 / ((onlyDouble[1][j][0] - onlyDouble[1][j][3])/2);
        ...
        System.out.println("Przenikanie (wzrost do polowy poprzedniej swiecy spadkowej)");
